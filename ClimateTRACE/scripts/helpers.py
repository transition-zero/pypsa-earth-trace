...
def get_modelling_progress(
    ...
    progress_data = pd.read_csv(
        "ClimateTRACE/outputs/model-benchmarks.csv",
    ...
    if save:
        progress_data.to_csv("ClimateTRACE/outputs/model-progress.csv", index=False)

        upload_blob(
            bucket_name="feo-pypsa-staging",
            source_file_name="ClimateTRACE/outputs/model-progress.csv",
...
def make_tracker_sheet(
    ...
    # to csv
    if save:
        trace_tracker.to_csv("ClimateTRACE/outputs/model-benchmarks.csv", index=False)
