...
from helpers import get_country_networks_from_bucket, get_modelling_progress
...
def plot_map():
    ...
    df = pd.read_csv("ClimateTRACE/outputs/model-progress.csv")
    ...
    fig.write_html("ClimateTRACE/outputs/progress-map.html")
    fig.write_image("ClimateTRACE/outputs/progress-map.png")
...
class CountryBenchmarking:
    ...
    def __init__(self, country_iso, fetch=False):
        ...
        self.historic_generation = pd.read_csv(
            "../data/ember-electricity-generation-monthly.csv"
        ).query("entity_code == @country_iso")

        # get historic demand data
        self.historic_demand = pd.read_csv(
            "../data/ember-electricity-demand-monthly.csv"
        ).query("entity_code == @country_iso")

        # get historic capacity data
        self.historic_capacity = pd.read_csv(
            "../data/ember-electricity-capacity-yearly.csv"
        ).query("entity_code == @country_iso")

        # get latest data from bucket
        if fetch:
            get_country_networks_from_bucket(
                bucket_name="feo-pypsa-staging", country_iso=country_iso
            )

        self.n_unconstr = pypsa.Network(
            f"../feo-pypsa-staging/results/{country_iso}/elec_s_10_ec_lv1.00_1H_trace.nc"
        )
        self.n_annual_matching = pypsa.Network(
            f"../feo-pypsa-staging/results/{country_iso}/elec_s_10_ec_lv1.00_1H-constr_trace.nc"
        )
    ...
    def get_total_tech_generation(
        ...
        """
        Get sum of generation for a given technology at a specified temporal
        resolution (e.g., 'D', 'M', 'Y')
        """
        year = str(year)
        ...
        return (
            pd.concat(
                [
                    ...
                    self.n_annual_matching.generators_t.p.filter(regex=tech_pypsa)
                    .sum(axis=1)
                    .resample(resample)
                    .sum(),
                ],
            ...
            .assign(
                historic=self.historic_generation.query(
                    " series == @tech and date.str.contains(@year) "
                )
            ...
            .reset_index()
            .melt(
                id_vars="snapshot", var_name="Constraint", value_name="Generation (MWh)"
            )
        )
    ...
    def get_capacity(self, year=2019):
        """
        Returns a pandas dataframe containing the historic and modelled
        capacity for a given year.
        """
        return pd.concat(
            [
                (
                    self.historic_capacity.query("date == @year")
                    .pivot_table(columns="series", values="capacity_gw", aggfunc="max")
                    .set_index(pd.Index(["Ember"]))
                    .round(1)
                ),
                (
                    self.n_unconstr.generators.groupby(by="carrier")
    ...
    def get_demand(self, year):
        ...
        return pd.DataFrame(
            {
                "Ember": [
                    self.historic_demand.query("date.str.contains(@year)")
    ...
    def plot_demand(self, year):
        ...
        ax = sns.barplot(data=demand_data, x="variable", y="value", ax=ax)
...
if __name__ == "__main__":
    ...
    failed_iso = []
    for cfig in os.listdir("ClimateTRACE/configs"):
        # get iso code
        ...
        for tech in ["coal", "solar", "gas", "nuclear"]:
            try:
                ...
                f, ax = plt.subplots()
                # plot
                country_data.plot_total_tech_generation(
                    tech=tech, year=2019, resample="M", ax=ax
                )
                # formatting
                ax.set_xlabel("Month")
                ...
                # save
                f.savefig(
                    f"ClimateTRACE/outputs/generation-plots/{iso_code}_{tech}_generation.png",
                    bbox_inches="tight",
                )
            except:
                pass
        ...
        f.savefig(
            f"ClimateTRACE/outputs/demand-plots/{iso_code}_demand_2019.png",
            bbox_inches="tight",
        )
        ...
        f.savefig(
            f"ClimateTRACE/outputs/capacity-plots/{iso_code}_capacity_2019.png",
            bbox_inches="tight",
        )
